  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const newBounty: Bounty = {
        ...
        author: ((data as Record<string, unknown>).anonymous_author as string) || "You",
      ...
      <div className="max-w-3xl mx-auto">
        ...
        {showForm && (
          <form onSubmit={handleSubmit} className="mb-8 p-6 bg-surface-raised rounded-2xl border border-white/10 animate-slide-up">
            ...
            <button
              type="submit"
              disabled={submitting}
              className="px-6 py-3 rounded-xl bg-brand text-white font-semibold hover:bg-brand-dark transition-colors border border-brand/30 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
              {submitting ? (
                <>
        ...
        {loading ? (
          <div className="space-y-4">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="bg-surface/50 border border-white/10 rounded-2xl p-6 animate-pulse">
          ...
          <div className="text-center py-20">
            <p className="text-muted text-lg mb-2">No active bounties yet</p>
            <p className="text-muted-faint text-sm">Be the first to post one!</p>
          </div>
        ) : (
          <div className="space-y-4">
            {bounties.map((bounty) => (
